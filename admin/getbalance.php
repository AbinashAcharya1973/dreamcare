...
$sql = "SELECT sum(income) as tincome,sum(withdrawal) as twithdraw FROM ledger where memid=" . $memid;
$result = $conn->query($sql);
$data = array();
if ($result->num_rows > 0) {
    if ($rec = $result->fetch_assoc()) {
        $balance=$rec['tincome']-$rec['twithdraw'];
        $data[] = array("ClosingBalance"=>$balance);
    }
    //mysql_close($conn);
