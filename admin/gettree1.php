...
function getGoJSData($conn, $startMemid, &$nodes, &$links) {
    $sql = "SELECT memid, mname, sponsorid,bankac,ifsc,emailid FROM members WHERE sponsorid = $startMemid";
...
$result=$conn->query("select * from members where memid=1'");
if($result->num_rows > 0) {
...
$nodes[]=$node;
getGoJSData($conn, 1, $nodes, $links);
