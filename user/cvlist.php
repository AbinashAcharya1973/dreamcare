                    ...
                    <div class="row">
                        ...
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-chart-area mr-1"></i>Payment Vouchers                                
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col text-left">
                                        <button class="btn btn-primary btn-sm btn-outline" v-on:click='newvoucher'>Add New Payment Voucher</button>
                                    </div>                                
                                </div>
                                <div class="row">
                                    <div class="col">
                                    ...
                                    <tbody>
                                        <tr v-for='(voucher,index) in PVoucherList'>
                                            <td>{{voucher.SlNo}}</td>
                                            <td>{{voucher.PDate}}</td>
            ...
            var app=new Vue({
                ...
                mounted:function(){
                    axios.post('gtcv')
                            .then(function(response){
                                app.PVoucherList=response.data;
                ...
                methods:{
                    newvoucher:async function(){
                        app.auth=await app.chkauth("Contra","Save");
						if(app.auth){
							window.location.assign('cvoucher');
						}
                    },
                    edit:async function(id){
						app.auth=await app.chkauth("Payment","Edit");
						if(app.auth){
							var tempid=app.PVoucherList[id].SlNo;
							localStorage.temprvno=tempid;
							window.location.assign('editpv');
						}
                    }
                }
            });            
